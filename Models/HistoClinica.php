...
class HistoClinica
{
	...
	public static function allByUser($usuario){
		$listaHistorias =[];
		$db=Db::getConnect();
		$sql=$db->prepare('SELECT hc.id, hc.fregistro, hc.numero, hc.paciente FROM histoclinicas hc, pacientes p WHERE hc.paciente = p.id AND p.usuario = '.$usuario.' ORDER BY hc.id');
		//hc INNER JOIN pacientes p WHERE p.usuario=:usuario
	...
	//la función para obtener una HC por el numero, usuario, y paciente
	public static function allByPaciente($histoclinica){
		$listaHistorias =[];
		$db=Db::getConnect();
		$query = 'SELECT hc.id, hc.fregistro, hc.numero, hc.paciente FROM histoclinicas hc, pacientes p WHERE hc.paciente = p.id ';
		$filtro = '';
		
		if(array_key_exists('usuario',$histoclinica) && !empty($histoclinica['usuario'])){
			$filtro.=' AND p.usuario = '.$histoclinica['usuario'];			
		}
		if(array_key_exists('numero',$histoclinica) && !empty($histoclinica['numero'])){
			$filtro.=' AND hc.numero = '.$histoclinica['numero'];
		}
		if(array_key_exists('nombre',$histoclinica) && !empty($histoclinica['nombre'])){
			$filtro.=' AND p.nombres LIKE \'%'.$histoclinica['nombre'].'%\'';
		}
		if(array_key_exists('apellido',$histoclinica) && !empty($histoclinica['apellido'])){
			$filtro.=' AND p.apellidos LIKE \'%'.$histoclinica['apellido'].'%\'';;
		}
		...
		foreach ($sql->fetchAll() as $historiaDb) {
			$listaHistorias[]= new HistoClinica($historiaDb['id'],$historiaDb['fregistro'],$historiaDb['numero'],$historiaDb['paciente']);
		}
		return $listaHistorias;
	}	
}