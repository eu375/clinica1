...
class Paciente
{
	...
	//función para obtener todos los pacientes por usuario
	public static function allByFilter($paciente){
		$listaPacientes =[];
		$db=Db::getConnect();
		$query = 'SELECT * FROM pacientes';
		$filtro = '';

		if(array_key_exists('usuario',$paciente) && !empty($paciente['usuario'])){
			$filtro.=' AND usuario = '.$paciente['usuario'];			
		}
		if(array_key_exists('dui',$paciente) && !empty($paciente['dui'])){
			$filtro.=' AND cedula = '.$paciente['dui'];
		}
		if(array_key_exists('nombre',$paciente) && !empty($paciente['nombre'])){
			$filtro.=' AND nombres LIKE \'%'.$paciente['nombre'].'%\'';
		}
		if(array_key_exists('apellido',$paciente) && !empty($paciente['apellido'])){
			$filtro.=' AND apellidos LIKE \'%'.$paciente['apellido'].'%\'';;
		}
