...
class Consulta
{
	...
	public static function allByUser($usuario){
		$listaConsultas =[];
		$db=Db::getConnect();
		$query = 'SELECT c.id, c.fecha, c.enfactual, c.diagnostico, c.diagnostico, c.paciente FROM consultas c, pacientes p WHERE c.paciente = p.id AND p.usuario = '.$usuario.' ORDER BY c.id';
	...
	public static function allByHisto($consultas){
		$consult =[];
		$db=Db::getConnect();
		$query = 'SELECT c.id, c.fecha, c.enfactual, c.diagnostico, c.diagnostico, c.paciente FROM consultas c , pacientes p ,histoclinicas h WHERE c.paciente = p.id AND p.id = h.paciente ';
		$filtro = '';
		
		if(array_key_exists('usuario',$consultas) && !empty($consultas['usuario'])){
			$filtro.=' AND p.usuario = '.$consultas['usuario'];	
			}
		if(array_key_exists('numero',$consultas) && !empty($consultas['numero'])){
			$filtro.=' AND h.numero = '.$consultas['numero'];
			}	
		if(array_key_exists('fecha',$consultas) && !empty($consultas['fecha'])){
			$filtro.=' AND c.fecha = '.$consultas['fecha'];
		}		
		if(array_key_exists('nombre',$consultas) && !empty($consultas['nombre'])){
			$filtro.=' AND p.nombres LIKE \'%'.$consultas['nombre'].'%\'';
		}
		if(array_key_exists('apellido',$consultas) && !empty($consultas['apellido'])){
			$filtro.=' AND p.apellidos LIKE \'%'.$consultas['apellido'].'%\'';;
		}

		$query.=$filtro;
		$sql=$db->prepare($query);		
		$sql->execute();
		// carga en la $listaHistoriales cada registro desde la base de datos
		foreach ($sql->fetchAll() as $consultaDb) {
			$consult[]= new Consulta($consultaDb['id'],$consultaDb['fecha'],$consultaDb['enfactual'],$consultaDb['diagnostico'],$consultaDb['diagnostico'],$consultaDb['paciente']);
		}
		return $consult;
	}	
