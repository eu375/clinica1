...
<h1>Resumen Historias Clínicas</h1>
	<form class="form-inline" action="?controller=historia&action=buscarExtendida" method="post">
	...
	<div class="form-group row">
	 <div class="col-xs-4">
	 	<button type="button" class="btn btn-success" onclick="location.href='?controller=historia&action=show'"><span class="glyphicon glyphicon-erase"></span> Limpiar</button>
	  </div>
	</div>	
	</form>
		...
		<table class="table table-hover">
			...
			<?php foreach ($lista_historias as $historia) {?>		
				<tr>
					...
					<td> <button type="button" class="btn btn-info" onclick="location.href='?controller=historia&action=reporteHistorico&id=<?php echo $paciente->getId()?>'" formtarget="_blank"><span class="glyphicon glyphicon-zoom-in"></span> Ver Histórico</button></td>
